...
message("\nCreating example tables and plots...")

#
# 10A) Look at the head of predictions
#
head(combined_predictions)

#
# 10B) Summarize RMSE & MAE across priors and sectors
#     (We already have combined_errors with columns: model, Sector, RMSE, MAE, MAPE, etc.)
#
# A simple table of (model, Sector, RMSE, MAE), excluding the "ALL" row if you like
error_table <- combined_errors %>%
  filter(Sector != "ALL") %>%
  dplyr::select(model, Sector, RMSE, MAE, MAPE) %>%
  arrange(model, Sector)

# Print to console
message("\nError Table (RMSE, MAE, MAPE) by Model & Sector:")
print(error_table,n=100)

error_table %>% group_by(model) %>% summarise(mean_rmse=mean(RMSE),mean_mae=mean(MAE))


# If you're in an environment that supports knitr::kable:
# (uncomment these lines if you want a nicely formatted table in an RMarkdown doc)
# knitr::kable(error_table, caption = "Error Metrics by Model & Sector")

#
# 10C) Summaries of parameter shrinkage
#
if (nrow(combined_param_summaries) > 0) {
  
  # A straightforward table of the posterior means and intervals
  # showing param_name, prior (model), mean, q5, q95, etc.
  param_table <- combined_param_summaries %>%
    dplyr::select(model, variable, mean, sd, q5, q95) %>%
    arrange(variable, model)
  
  message("\nParameter Summary Table:")
  print(param_table)
  
  # If using RMarkdown, you can optionally do:
  # knitr::kable(param_table, caption = "Posterior Parameter Estimates by Prior")
  
} else {
  message("\nNo parameter summaries available.")
}


unique_sectors <- unique(combined_predictions$Sector)

unique_sectors_s1 <- unique_sectors[1:6]

unique_sectors_s2 <-  unique_sectors[7:11]

#
# 10D) Plot forecast vs actual for each prior
#
p1 <- ggplot(combined_predictions %>% filter(Sector %in% unique_sectors_s1), aes(x = Date, y = mean, color = forecast, group = forecast)) +
  geom_line(size = 1) +
  facet_wrap(~ Sector + model, scales = "free_y") +
  theme_minimal() +
  labs(
...
p1.2 <- ggplot(combined_predictions %>% filter(Sector %in% unique_sectors_s2), aes(x = Date, y = mean, color = forecast, group = forecast)) +
  geom_line(size = 1) +
  facet_wrap(~ Sector + model, scales = "free_y") +
  theme_minimal() +
  labs(
...
#
# 10E) Bar plot of RMSE by prior and sector
#
p2 <- ggplot(combined_errors %>% filter(Sector != "ALL"), 
...
#
# 10F) Parameter summaries (posterior means) plot
#
if (nrow(combined_param_summaries) > 0) {
  ...
  # OPTIONAL ALTERNATIVE: Grouped bar chart with error bars (q5, q95)
  # (uncomment to try)
  #
  # p3_alt <- ggplot(combined_param_summaries, aes(x = variable, y = mean, fill = model)) +
  #   geom_bar(stat = "identity", position = position_dodge(width = 0.7)) +
  #   # Add error bars from q5 to q95:
  #   geom_errorbar(
  ...
  #   theme_minimal() +
  #   labs(
  #     title = "Parameter Estimates (Posterior Means + 90% CI) by Prior",
  #     x     = "Parameter",
  #     y     = "Posterior Mean"
  #   ) +
  #   theme(axis.text.x = element_text(angle = 45, hjust = 1))
...
# ------------------------------------------------------------------------------
# 10B) Create a wide table of parameter means by model
# ------------------------------------------------------------------------------
if (nrow(combined_param_summaries) > 0) {
  
  message("\n=== Parameter Summaries ===")
  
  # 1. Pivot from long -> wide to compare posterior means across priors
  param_summary_means_wide <- combined_param_summaries %>%
    dplyr::select(model, variable, mean) %>%
    tidyr::pivot_wider(
      names_from  = model,       # e.g. "hierarchical", "laplace", ...
      values_from = mean         # the column you want in wide format
    )
  
  # Show a few rows
  message("\nWide Table of Posterior Means (each row = parameter, each column = model):")
  print(head(param_summary_means_wide, 20))  # or however many rows you want to see
  
  # 2. Parse out base parameter name (e.g. "A", "beta") and the indices in brackets
  #    This helps you filter or group parameters by type.
  param_summary_means_wide_parsed <- param_summary_means_wide %>%
    mutate(
      param_type = stringr::str_extract(variable, "^[^\\[]+"),            # everything before '['
      param_idx  = stringr::str_extract(variable, "(?<=\\[).+?(?=\\])")   # the stuff in brackets
    ) %>%
    relocate(param_type, param_idx, .before = variable)
  
  message("\nParsed version (splits param name & index):")
  print(head(param_summary_means_wide_parsed, 20))
  
  # (Optional) If you want to specifically look at, say, "A" parameters, do:
  # filter(param_summary_means_wide_parsed, param_type == "A")
  
  # 3. Print a small excerpt of parameter summaries with mean, sd, q5, q95
  #    (still in long format) to see everything at once.
  param_table_verbose <- combined_param_summaries %>%
    arrange(variable, model) %>%
    dplyr::select(model, variable, mean, sd, q5, q95)
  
  message("\nDetailed (long) parameter table (first 20 rows):")
  print(head(param_table_verbose, 20))
  
  # 4. Identify which parameters vary the most across priors in their posterior means.
  #    For example, you can group by parameter name and see the max - min range:
  param_largest_diff <- combined_param_summaries %>%
    group_by(variable) %>%
    summarise(
      range_in_mean = max(mean) - min(mean),
      # Optionally keep track of which model is max or min
      max_model = model[which.max(mean)],
      min_model = model[which.min(mean)],
      .groups = "drop"
    ) %>%
    arrange(desc(range_in_mean))
  
  message("\nParameters with the largest difference in posterior means across priors:")
  print(head(param_largest_diff, 20))
  
} else {
  message("\nNo parameter summaries to tabulate or parse.")
}
...
install.packages("kableExtra")  # if not already installed
...
# A) Filter out "ALL" if you don't want that row, then drop MAPE
error_table_for_latex <- error_table %>%
  dplyr::filter(Sector != "ALL") %>%
  dplyr::select(model, Sector, RMSE, MAE)

# B) Create a LaTeX table using knitr::kable and kableExtra
sector_error_latex <- knitr::kable(
  error_table_for_latex,
  format    = "latex",
  booktabs  = TRUE,
  caption   = "Sector-Level RMSE and MAE by Model",
  label     = "table:sector-errors",  # for LaTeX referencing
  digits    = 4  # number of decimal places
) %>%
  kable_styling(latex_options = c("striped", "scale_down"))

# C) Print it to the console (in R Markdown, it will appear in the rendered PDF)
sector_error_latex
