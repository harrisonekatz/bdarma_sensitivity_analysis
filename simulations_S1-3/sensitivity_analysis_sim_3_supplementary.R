...
load("sim_2_matrices_large.RData")
...
fit_model <- function(y_train, prior_type, stan_model_file, y_test, true_params, X_t_train, X_t_test) {
  ...
  beta_draws <- posterior_df %>%
    dplyr::select(matches("^beta")) %>%                # Select all columns starting with 'beta'
    dplyr::select(-matches("^beta_"))
  
  # Refine B_draws to only include 'B' without including 'beta'
  B_draws <- posterior_df %>%
    dplyr::select(matches("^B")) %>%                   # Select all columns starting with 'B'
  ...
  A_estimates <- colMeans(A1_samples)
  bias_beta <- beta_estimates - true_params$beta
  bias_A1 <- A_estimates - as.vector(true_params$A1)
...
for (sim in 1:num_simulations) {
  ...
  while (!success) {
    tryCatch({
      ...
      # Covariate data
      X_t <- matrix(data = c(rep(1, 5 * T_total)), ncol = 5)
      
      # Split data into training and test sets
      y_train <- y[1:T_train, ]
      y_test  <- y[(T_train + 1):T_total, ]
      
      # These lines would typically be X_t[1:T_train, , drop = FALSE] 
      # to avoid dimension issues, but if you truly want vectors, you can keep as is:
      ...
      # If we reach here without an error, we're successful
      success <- TRUE
      print(sim)
      ...
      cat("Rerunning simulation", sim, "...\n\n")
      success <- FALSE
      # Optionally do any clean-up or logging here before retrying
    })
  }
}
